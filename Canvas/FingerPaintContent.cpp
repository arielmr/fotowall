...
FingerPaintContent::FingerPaintContent(bool spontaneous, QGraphicsScene * scene, QGraphicsItem * parent)
...
bool FingerPaintContent::loadPhoto(const QString & picFilePath, bool keepRatio, bool setName)
{
    dropNetworkConnection();
    delete m_photo;
...
bool FingerPaintContent::loadFromNetwork(const QString & url, QNetworkReply * reply, const QString & title, int width, int height)
{
    dropNetworkConnection();
    delete m_photo;
    m_cachedPhoto = QPixmap();
...
bool FingerPaintContent::loadPixmap(const QPixmap & pixmap, const QString & title)
{
    dropNetworkConnection();
    delete m_photo;
    m_cachedPhoto = QPixmap();
...
void FingerPaintContent::addEffect(const PictureEffect & effect)
{
    if (!m_photo)
        return;
...
QWidget * FingerPaintContent::createPropertyWidget(ContentProperties * __p)
{
...
bool FingerPaintContent::fromXml(QDomElement & contentElement, const QDir & baseDir)
{
...
void FingerPaintContent::toXml(QDomElement & contentElement, const QDir & baseDir) const
{
...
QPixmap FingerPaintContent::toPixmap(const QSize & size, Qt::AspectRatioMode ratio)
{
    if (m_photo)
...
int FingerPaintContent::contentHeightForWidth(int width) const
{
    if (m_netWidth > 0 && m_netHeight > 0)
...
bool FingerPaintContent::contentOpaque() const
{
    return m_opaquePhoto;
}

void FingerPaintContent::dropEvent(QGraphicsSceneDragDropEvent * event)
{
...
bool FingerPaintContent::externalEdit() const
{
    return m_watcher;
}
...
void FingerPaintContent::applyPostLoadEffects()
{
...
void FingerPaintContent::slotGimpCompressNotifies()
{
    if (!m_watcherTimer) {
...
void FingerPaintContent::slotGimpFinished()
{
...
bool FingerPaintContent::slotLoadNetworkData()
{
    // get the data
    if (!m_netReply)
        return false;
...
void FingerPaintContent::slotNetworkError()
{
    // clear state
    if (!m_netReply)
        return;
...
void FingerPaintContent::slotNetworkProgress(qint64 a, qint64 b)
{
